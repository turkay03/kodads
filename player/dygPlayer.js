(function() {
	...
	this.DygPlayer = function() {
         ...
         loginStatus,music,nextChannelLogo,nextChannelUrl,partDuration,partner,performanceMode,playbackDevice,playbackTime,refGo,seedTrackingUrl,seedUrl,selectPart,clearCache,videoRatio,/*new params*/cinemaMode,forceStaticConfig,isLive,scaleMode,pathName,playerType,playerTheme,splitParts,usePartition,controlName,nextVideo,cinemaState,customPackage/*new params*/;  
    ...
    var obj = {
     ...
     addPlayer:function(){
      ...
      for(var i=0;i<=videoCount-1;i++){
		...
		forceStaticConfig=playerArray[i]["forceStaticConfig"];
		customPackage=playerArray[i]["customPackage"];
		cinemaMode = playerArray[i]["cinemaMode"];
		cinemaState=0;
		...
		if(playerTheme=="vogue"){
			playerConfig.controlName="flowplayer.controls-vogue-3.2.16.swf";
		}
		if(playerTheme=="glamour"){
			playerConfig.controlName="flowplayer.controls-glamour-3.2.16.swf"; 
		}
		if (playerTheme == "star") {
            playerConfig.controlName = "flowplayer.controls-star-3.2.16.swf";
			playerTheme="";
        }
          ...
          case "vouge":
           ...
           default:
						...
						if(autoPlay==null){ //autoplay boş geldiğinde true
						 ...
						 if(flashVersion==0 || flashStatus){
								...
								if(navigator.userAgent.match(/iPhone/i)){
									  document.getElementById(divId).innerHTML = "<video width=\"100%\" height=\"100%\"  autoplay=\"true\" controls x-webkit-airplay=\"allow\" id=\""+divId+"_1_html5_api"+"\"><source src="+mobileUrl+"></video>";
								}else{
								...
								console.log("den01");
										...
										if(cinemaMode!=null){
											...
											videojs.newButton.prototype.onTap = function() {
												if(cinemaState==0){
													window[cinemaMode.openCallBack]();
													cinemaState=1;
													this.q.el.className="vjs-new-button-off vjs-control"
												}else{
													window[cinemaMode.closeCallBack]();
													cinemaState=0;
													this.q.el.className="vjs-new-button vjs-control"
												}
											};
											...
											var createNewButton = function() {
												var props = {
													...
													innerHTML: '<div class="vjs-control-content"><span class="vjs-control-text">Sinema</span></div>',
													role: 'button',
													'aria-live': 'polite', 
													tabIndex: 0
													};
										 ...
										 player.on("firstplay", function(den) {
											 	addParamForLessOneMin(den.target.firstChild.duration);
														...
														try {
															if(playerConfig.identifier!=""){
																gemiusStream.newStream(playerConfig.playerId, mobileUrl, den.target.firstChild.duration,customPackage,playerConfig.additionalPackage, playerConfig.identifier, playerConfig.hitcollector, null);
																gemiusStream.event(playerConfig.playerId, mobileUrl, 0, "playing");
															}	
														}
														catch(err) {
										...
										var options = {
												  id: divId,
												  adTagUrl: decodeURIComponent(playerConfig.adLib[0].adURL),
												  locale:"tr"
												};
							...
							clip: {
									...
									forceStaticConfig:forceStaticConfig,
									cinemaMode:cinemaMode,
									hideAds:hideAds, 
									adLib: playerConfig.adLib,
									start: '0',
									duration:'0',
									onStart: function(clip) {
										...
										sendPlayEvent=true;
										addParamForLessOneMin(clip.duration);
										...
										try {
											if(playerConfig.identifier!=""){
												gemiusStream.newStream(playerConfig.playerId, clip.originalUrl, clip.duration, customPackage,playerConfig.additionalPackage, playerConfig.identifier, playerConfig.hitcollector, null);
							...
							plugins: {
								...
								controls: {
									url: libPath+playerConfig.controlName+"?0011"
								},			
								ima: {
			...
			pause:function(plaID){
			 ...
			 getTime: function(plaID) {
				 try {
					if (flashVersion == 0 || flashStatus) {
						return document.getElementById(plaID+"_1_html5_api").currentTime;
					}else{
						return $f(plaID).getTime();
					}
				} catch (err) {}
			  },
			...
			showControls:function(plaID){
			 ...
			 getState: function(plaID) {
				try {
					if (flashVersion == 0 || flashStatus) {
						if(document.getElementById(plaID+"_1_html5_api").paused){
							return 4;
						}else{
							return 3;
						}
					}else{
						if($f(plaID).getTime()==currentTime){
							return 4;
						}
						currentTime=$f(plaID).getTime();
						return $f(plaID).getState();
					}
				} catch (err) {}
			  },
			  getDuration: function(plaID) {
				try {
					if (flashVersion == 0 || flashStatus) {
						return document.getElementById(plaID+"_1_html5_api").duration;
					}else{
						return $f(plaID).getClip(0).duration;
					}
				} catch (err) {}
			  }
	  };
...
function completeScript(callback){
	...
	function getConfig(){
		var obj={
			...
			materialIdentifier:'',
			totalTime:0,
			customPackage:'', //Her video için ayrı olduğundan Video parametreleri üzerinden alınıyor.
			additionalPackage:'',
			treeId:'',
			dfpSiteName:'-',
			controlName:'flowplayer.controls-3.2.16.swf'
		...
		try{
			if(DygDFP){  
				obj.dfpSiteName="Web/"+DygDFP.siteName;
			}
		}catch(e){
				//console.log("Error: dygBase not loaded");
				DygDFP = {
		...
		try{
			...
			if (DygDFP){
				...
				obj.adLib=[
				{
					time:DygDFP.VideoRequests()[0].time,
					adURL:encodeURIComponent(DygDFP.VideoRequests()[0].url).replace(/'/g, " "),
					useSkip:false
				},
				{
					time:DygDFP.VideoRequests()[1].time,
					adURL:encodeURIComponent(DygDFP.VideoRequests()[1].url).replace(/'/g, " "),
					useSkip:false,
					closeButton:true
				},
				{
					time:DygDFP.VideoRequests()[2].time,
					adURL:encodeURIComponent(DygDFP.VideoRequests()[2].url).replace(/'/g, " "),
					useSkip:false
				},
				{
					time:DygDFP.VideoRequests()[3].time,
					adURL:encodeURIComponent(DygDFP.VideoRequests()[3].url).replace(/'/g, " "),
					useSkip:false
				}]			
			}		
		}catch(e){
		...
		switch(true) {
			...
			case /localhost/.test(document.domain):
				...
				obj.identifier='1v1FBqMIdUIl7pQCbuCb4NVqrhy1xU9HCfz_v_GzY_v.E7';
				break;	
			case /tivibuspor.com.tr/.test(document.domain):
				obj.dkey='#$dd347e29d9694d7ff1b';
				break;		
			case /tivibuspor.com/.test(document.domain):
				obj.dkey='#$d31a80c13a7b32d4b8f';
				break;		
			case /tlctv.com.tr/.test(document.domain):
				obj.dkey='#$b1d44586906d28999d5';
				break;
			case /glamour.com.tr/.test(document.domain):
				obj.dkey='#$6d9b1d8576c7fc6200b';
				break;	
			default:
				obj.dkey='';
		}
